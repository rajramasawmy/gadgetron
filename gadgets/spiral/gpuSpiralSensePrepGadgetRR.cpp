#include "gpuSpiralSensePrepGadgetRR.h"
...
namespace Gadgetron{
  ...
  gpuSpiralSensePrepGadgetRR::gpuSpiralSensePrepGadgetRR()
  ...
  int gpuSpiralSensePrepGadgetRR::process_config(ACE_Message_Block* mb)
  {
    ...
    //
    RR_rss_ = RR_rss.value();
    vds_factor_ = vds_factor.value();
  ...
  int gpuSpiralSensePrepGadgetRR::
    ...
    csm_ = estimate_b1_map<float,2>( &image ); // Estimates csm
     ...
     if(RR_rss_){
       // ********* altRR *********
   	  ...
   	  boost::shared_ptr< hoNDArray<float_complext> >image_RR;
   	  image_RR = image.to_host();
   	  hoNDArray<Gadgetron::complext<float>> test_image_RR3 = hoNDArray<Gadgetron::complext<float>>(image_dims);//(img_dims_RR);
   	  test_image_RR3 = *image_RR;

   	  // Use image data to generate Mackenzie CSM CSM_M=(img./ sqrt(sum(img.*conj(img),<coils>))
   	  std::vector<size_t> img_dims_RR;
   	  img_dims_RR.push_back(image_dimensions_recon_[0]);
   	  img_dims_RR.push_back(image_dimensions_recon_[1]);

   	  // weighted image set-up
   	  hoNDArray<Gadgetron::complext<float>> chunk_RR = hoNDArray<Gadgetron::complext<float>>(image_dims,&test_image_RR3(0,0,0));
   	  hoNDArray<Gadgetron::complext<float>> chunk_RR2 = hoNDArray<Gadgetron::complext<float>>(image_dims);

   	  multiplyConj(chunk_RR, chunk_RR, chunk_RR2);

    	  hoNDArray<Gadgetron::complext<float>> output = hoNDArray<Gadgetron::complext<float>>(img_dims_RR);
   	  clear(output); //zero values

   	  // sum over coils and sqrt
   	  for (int i1=0; i1 < num_coils; ++i1){
   	    //  GDEBUG("###RR: i1: %d\n", i1);
   	    output += hoNDArray<Gadgetron::complext<float>>(img_dims_RR, &chunk_RR2(0,0,i1));
   	  }
   	  //	  Gadgetron::sqrt(output,output);
   	  sqrt_inplace(&output);


   	  // Element-wise division
   	  //	  hoNDArray<Gadgetron::complext<float>> test_im2 = hoNDArray<Gadgetron::complext<float>>(image_dims);
   	  hoNDArray<Gadgetron::complext<float>> csm_RR_2 = hoNDArray<Gadgetron::complext<float>>(image_dims);

   	  // Coil-wise normalization
   	  for (int i1=0; i1 < num_coils; ++i1){
   	    hoNDArray<Gadgetron::complext<float>> a (img_dims_RR, &test_image_RR3(0,0,i1));
   	    hoNDArray<Gadgetron::complext<float>> b (img_dims_RR, &csm_RR_2(0,0,i1));
   	    Gadgetron::divide(a, output,b);
   	  }

   	  // Upload to CSM
   	  cuNDArray<float_complext> a (csm_RR_2);
   	  *csm_ = a;
   	  // *********************

   	  // RR write a copy to the host
/*   	  {
   	    static int counter = 0;
   	    char filename[256];
   	    sprintf((char*)filename, "_csm_RR_%d.cplx",counter);
   	    write_nd_array<float_complext>(&csm_RR_2, filename);
   	    //sprintf((char*)filename, "_image_RR_%d.cplx",counter);
   	    //write_nd_array<float_complext>(*image_RR, filename);
   	    sprintf((char*)filename, "_csm_%d.cplx",counter);
   	    write_nd_array<float_complext>(csm_->to_host().get(), filename);
   	    counter++;
      } */
  ...
  gpuSpiralSensePrepGadgetRR::duplicate_profile( GadgetContainerMessage<ISMRMRD::AcquisitionHeader> *profile )
  {
  ...
  GADGET_FACTORY_DECLARE(gpuSpiralSensePrepGadgetRR)
}
